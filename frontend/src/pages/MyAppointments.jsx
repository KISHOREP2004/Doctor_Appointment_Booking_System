...
const MyAppointments = () => {
 ...
 useEffect(()=>{
  ...
  return (
    <div>
      <p className='pb-3 mt-12 font-medium text-zinc-7000 border-b'>My appointments</p>
      <div>
        {appointments.map((item,index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-bottom' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && item.payment && !item.isCompleted && <button className='sm:min-48 py-2 border rounded  text-stone-600 bg-green-400'>Paid</button>}
           ...
           { !item.cancelled &&  !item.isCompleted &&  <button onClick={()=>cancelAppointment(item._id)}   className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-red-500 hover:text-white transition-all duration-300'>Cancel appointment</button>}
            {item.cancelled && !item.isCompleted &&  <button className='sm:minw-48 py-2 border border-red-500 rounded text-red-500'>Appointment cancelled</button>}
